...
ResidualModelCoMVelocityTpl<Scalar>::ResidualModelCoMVelocityTpl(
    boost::shared_ptr<StateMultibody> state, const Vector3s& vref,
    const std::size_t nu)
...
template <typename Scalar>
void ResidualModelCoMVelocityTpl<Scalar>::calc(
...
template <typename Scalar>
void ResidualModelCoMVelocityTpl<Scalar>::calcDiff(
    const boost::shared_ptr<ResidualDataAbstract>& data,
...
ResidualModelCoMVelocityTpl<Scalar>::createData(
...
ResidualModelCoMVelocityTpl<Scalar>::get_reference() const {
  return vref_;
}
