...
class ResidualModelAnkleLimits(croc.ResidualModelAbstract):
    ...
    def calc(self, data, x, u):
        ...
        self.ankleActuator2.actuation(
            x[: self.ankleActuator2.model.nq], np.zeros(self.ankleActuator2.nu)
        )
        self.m_1 = self.ankleActuator1.m
        self.m_2 = self.ankleActuator2.m
        data.r = (
            np.array([float(self.m_1), float(self.m_2)]).flatten()
            - np.array(self.delta_m).flatten()
        )
